...
- **Responsive/PWA: the "real app" (full-bleed, no phone-mockup chrome) now shows on any phone or tablet browser width, and on ANY width at all once launched as an installed PWA.** The breakpoint moved from `max-width:760px` to `max-width:900px` (covers iPad-portrait widths, 768–834px, without touching the 1200px-wide default test viewport) and gained a `(display-mode:standalone), (display-mode:fullscreen), (display-mode:window-controls-overlay)` OR-clause, so an installed desktop PWA window gets the real app regardless of how wide it is — a desktop install shouldn't show this session's own design-review wrapper around itself. `.fw-shell` also caps at `max-width:560px` and centers (`margin:0 auto`) inside that rule, so a wide installed window gets a comfortable centered column instead of the layout stretching edge to edge; this is a no-op on an actual phone (100% of ~390px is already under the 560px cap). **A plain desktop browser tab above 900px, not installed, is unchanged** — phone mockup + intro text + the dev screen-jump chips, same as always. That's deliberate: those chips are how this session navigates screens directly while testing (several Playwright suites click them, e.g. `page.locator('button', {hasText:/^Choose verses$/}).first()` hits the chip, not the real Home-screen button, because chips render first in DOM order) — don't widen the breakpoint further or remove the chips without checking every suite that relies on them.
