...
- **Step 1's illustration is a live demo, not an icon** — a real dashed-then-filled blank ("The LORD is my \_\_\_") with a word tile beneath it, looping through the actual tap-to-fill motion via three small keyframes (`demoTap`/`demoPop`/`demoFill`) built for this. It settles on the "before" frame (empty blank, tile visible) under `prefers-reduced-motion`, which is a legible static example on its own, not a broken mid-animation freeze. If the tap-to-fill mechanic ever changes, update this demo, not just the paragraph next to it.
