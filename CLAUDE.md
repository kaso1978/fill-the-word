...
- **Friends is the app's first real backend — everything else is still `localStorage`-only.** Two people connect and see each other's **verses memorized** (`Object.keys(state.mastered).length`, the same lifetime mastery map badges/Library already read — this is the first thing to also show it as its own labeled number) and **accuracy** (the existing `Math.round(correct/attempts*100)` formula, unchanged). Backend is **Supabase** (Postgres + Row Level Security) — a standing decision from before this feature existed (relational fit for "I see my own stats and my connected friends', nothing else"), not something picked for this feature specifically. Schema lives in `supabase/schema.sql`: a `profiles` table (id, display_name, friend_code, verses_memorized, accuracy) and a `friendships` table, both RLS-scoped so a plain `select * from profiles` from the client already comes back as just "me + my friends" with no explicit filtering in application code; redeeming a code goes through a `redeem_friend_code` Postgres function specifically so a client never needs broad read access to other users' rows just to look one up by code.
  ...
  - **A friend's full badges and Bible mastery are viewable, not just their verses-memorized/accuracy numbers.** Tapping a friend row (now a real button, `viewFriendProgress`) opens the *same* Progress screen you'd see for yourself — badge grid and Bible tab included — just sourced from their data instead of yours. This works by decoding what `syncFriends` already pushes (see below) into `state.viewingFriend: {id, name, mastered, badges}`, then two single-line swaps in `renderVals` (`activeMastered`, and `badges = viewingFriend ? viewingFriend.badges : computeBadges(s)`) feed the *exact same* 14 hardcoded badge tiles and the *exact same* per-book Bible list — no parallel UI to keep in sync. `computeBadges()` itself is never called on a friend's data (it needs raw streak/round-count fields that never leave your own device) — only the *already-computed tier snapshot* synced at their last refresh is used. The header shows "`<Name>`'s Progress" plus a "Back to Friends" button (`exitFriendProgress`); tapping the Progress tab itself also always clears `viewingFriend` (see `go()`), so "my progress" is never more than one tap away regardless of how you got into a friend's view.
  - **`mastered` and `badges` sync as compact `"key=value;key=value"` strings, not jsonb** (new `profiles` columns, no RLS change needed — the existing row-level `select` policies already cover new columns on the same row). Considered and rejected: jsonb (needlessly verbose — JSON's per-key quoting/colons cost real bytes at zero benefit here, since the client never needs to query *inside* these fields on the server side) and a bit-packed binary encoding keyed by a global verse index (meaningfully smaller only once a user has mastered a large fraction of the entire ~31,000-verse Bible — for the realistic case of a few hundred mastered verses, the fixed-size dense format is *larger* than the sparse one, and it adds a whole new "canonical global verse ordering" concept purely for a scale this app was never going to hit). The chosen format reuses the exact same keys already used locally (verse keys never contain `=`, so splitting each pair on its last `=` is unambiguous) — `encodeCompact`/`decodeCompact` are shared by both fields; `decodeBadges` additionally back-fills every badge not present in the string as `-1` (unearned), since only earned badges (tier > -1) are worth sending at all. Real-world size is a non-issue either way — see the size math from the conversation that led here: even mastering the *entire* Bible tops out under 1MB/user; the realistic case is 5-50KB. This was never a storage-size fix, just "don't pay JSON's tax for no reason."
...
- **Below 900px wide (phones, small/medium tablets in portrait, most installed-PWA windows), the app is full-bleed with zero chrome — that part of the "no phone-mockup" decision stands.** `.fw-page`/`.fw-shell`/`.fw-screen`/`.fw-status` render exactly as they always have under that width; nothing here changed.
- **At 900px and up (wide tablets in landscape, real desktop browser tabs), the app now presents as a floating card on a warm gradient backdrop instead of stretching the same narrow column edge-to-edge for the full window height.** A single `@media (min-width:900px)` block (right after the existing `.fw-tabs`/`.fw-bank` safe-area rules) — `.fw-shell` gets a `max-height` cap, rounded corners and a soft shadow (`!important` where an inline style needs overriding: `border-radius`, `box-shadow`; `max-height` and `.fw-page`'s `justify-content:center` don't, since nothing inline already sets those), `.fw-page` gets breathing-room padding, and `body`'s background becomes a radial gradient. This is **not** a return to the old design-review wrapper: no marketing copy, no visible dev tooling, no browser-chrome look — just the same app, framed. Reintroducing a breakpoint like this was a real reversal of the earlier "full-bleed at any width, no exceptions" call (see below), made deliberately this time after actually looking at how a 560px-wide, 100%-tall column looked stretched down a 1440×900 browser window: a decision the earlier full-bleed rework didn't have to weigh, since a Claude Artifact's iframe never gets that tall or wide.
  - **The backdrop's light/dark gradient keys off `.fw-shell[data-theme]` via `body:has(.fw-shell[data-theme="dark"])`, not `prefers-color-scheme`.** The app's own theme (light/dark/system, user-toggleable in Settings) can disagree with the OS's — `prefers-color-scheme` would mismatch the backdrop to the card's actual rendered theme whenever it does. `:has()` reads the shell's real resolved theme directly instead.
  - **900px, not 760px** (an earlier value this exact breakpoint used before "full-bleed at any width" removed it entirely, per the Test build section) — chosen so an iPad in portrait (810–820px, held like a big phone) still gets the immersive full-bleed treatment, and only landscape tablets and true desktop widths get the card.
  - **The hidden dev-nav is unaffected by any of this — verified, not assumed.** `.fw-chips` is `position:fixed` to the viewport, not scoped inside `.fw-shell`, and its visibility is driven entirely by `state.devNav`, never CSS media queries — there was never a mechanism here that could expose it at any width. Confirmed via Playwright at 1440px: no dev-nav present on load, the long-press-Settings gesture still reveals it exactly as before, Escape still closes it.
